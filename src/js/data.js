const data = [
    ...
    }, 
    {
      ...
      {
        "id": 32,
        "title": "Karen & Coffea (Codeforces 816B)",
        "content": "## Problem\nWe are given a set of n ranges $(l_i < r_i)$, and $k$ and $q$ queries. Each query asks about the amount of numbers from $a$ to $b$ that have appeared at least $k$ times in the above given ranges.\n\n## Solution\nA naive brute force approach goes as follows:\n1. A nested loop for maintaining a frequency array for each range.\n2. For each query, another nested loop for checking all the elements between $a$ and $b$ against that frequency array.\n\nThis gives us a time complexity of:\n$$ O(n \\cdot (r - l) + q \\cdot (b - a)) = O(n^2) $$\n\nLet's try to improve this using a bottom-up approach. Ideally, we want to process the queries in $O(1)$ time. We can utilize a prefix sum array to do this. To construct this array, we need to keep track of all the elements that satisfy the $k$ condition.\n\nThis leads us to our frequency array. We need to optimize our frequency array construction from $O(n^2)$ to at least $O(n)$.\n\nNow check out this cool trick:\n\nFor each $l$ and $r$, we will do:\n- $\\text{freqArr}[l]++$\n- $\\text{freqArr}[r+1]--$\n\nThis way we are only marking the bounds of the ranges in our freqArray, in $O(1)$ per each entry. To make this freqArray actually useful, we will do a prefix sum:\n$$ \\text{freqArr}[i] += \\text{freqArr}[i - 1] $$\n\n### Why does this work?\nLet's take an example of the ranges:\n- 1 3\n- 1 2\n- 2 4\n\nYou can see that for $i = 2$, it is correct to add to it the freq of $i = 1$. Why? Because the range starting with 1 will always include 2 in it. The 1's range will always have an $r$ value $\\geq 2$.\n\nNow that we have a working freqArray, we can make a boolean array which tells us which elements satisfy the $k$ condition or not. Then do a final prefix sum on that bool array. We can answer the queries in $O(1)$ time by:\n$$ \\text{preSum}[r] - \\text{preSum}[l - 1] $$\n\n### Code Solution:\n```cpp\n#include <iostream>\n#include <bits/stdc++.h>\n\nusing namespace std;\n\nint arr[200100] = {0};\nint preSums[200100] = {0};\nint main()\n{\n    ios_base::sync_with_stdio(false);\n    cin.tie(NULL);\n    int n, k, q;\n    cin >> n >> k >> q;\n\n    for (int i=0; i<n; i++)\n    {\n        int l, r;\n        cin >> l >> r;\n        arr[l] += 1;\n        arr[r+1] --;\n    }\n\n    for (int i=1; i<=200100; i++) {\n        arr[i] = arr[i-1] + arr[i];\n    }\n\n    for (int i = 1; i<=200100; i++)\n    {\n        if (arr[i] >= k)\n        {\n            preSums[i] = 1;\n        }\n        else\n        {\n            preSums[i] = 0;\n        }\n    }\n\n    for (int i=1; i<=200100; i++) {\n        preSums[i] = preSums[i-1] + preSums[i];\n    }\n\n    for (int i=0; i<q; i++)\n    {\n        int l, r;\n        cin >> l >> r;\n        cout <<  preSums[r]- preSums[l-1] << endl;\n    }\n    return 0;\n}\n```",
        "category": "problems",
        "resource": {
          "in": "../src/problems/cf_816_B.png",
          "out": "https://codeforces.com/contest/816/problem/B"
        },
        "author": "Mohamed Hany",
        "video": ""
      },
      {
        ...
        "content": "The **difference array** (also known as the **range update trick**) is a powerful technique, especially in competitive programming and algorithms, for performing **range updates in constant time** and then **reconstructing the final array** via a prefix sum.\n\n---\n\n## What Is a Difference Array?\n\nLet’s say you have an array `A` of size `n`. The **difference array** `D` of `A` is defined such that:\n\n$$\nD[0] = A[0], \\quad D[i] = A[i] - A[i-1] \\text{ for } i > 0\n$$\n\nGiven `D`, you can recover the original array `A` by doing a prefix sum:\n\n$$\nA[i] = D[0] + D[1] + \\dots + D[i]\n$$\n\n---\n\n## Why Use It?\n\nBecause with the difference array, you can perform **range updates** (i.e. add a value `x` to all elements from index `l` to `r`) in **constant time**, instead of looping through the range.\n\n---\n\n##  The Trick\n\nTo **add `x` to a subarray from `l` to `r`**, you update:\n\n```cpp\nD[l] += x;\nD[r+1] -= x;\n```\n\nThen, when you do a **prefix sum** on `D`, all the elements in `A[l..r]` will have been incremented by `x`.\n\n###  Why Does This Work?\n\nSuppose we apply the prefix sum to `D`:\n\n* For `i < l`: no change, because no updates affect this range.\n* From `i = l` to `i = r`: each element gets `+x`, because `D[l]` was incremented and keeps adding forward.\n* At `i = r+1`: we subtract `x` to stop the propagation.\n\n---\n\n##  Example\n\nLet’s say we start with an array of 0s:\n\n```txt\nA = [0, 0, 0, 0, 0]\n```\n\nNow let’s add `+3` to the range \\[1,3]:\n\n```cpp\nD[1] += 3;\nD[4] -= 3; // since r+1 = 3+1 = 4\n```\n\n`D` becomes:\n\n```txt\nD = [0, 3, 0, 0, -3]\n```\n\nNow apply prefix sum on `D`:\n\n```txt\nPrefix sum: [0, 3, 3, 3, 0]  → this is the final array A\n```\n\nWhich is exactly what we want: `+3` added to elements 1 to 3.\n\n---\n\n##  Time Complexity\n\n| Operation    | Normal Array                 | Difference Array                           |\n| ------------ | ---------------------------- | ------------------------------------------ |\n| Point Update | O(1)                         | O(1)                                       |\n| Range Update | O(n)                         | **O(1)**                                   |\n| Point Query  | O(1)                         | After prefix sum                           |\n| Range Query  | O(n) or Fenwick/Segment Tree | Still need prefix sum or another structure |\n\nSo it’s perfect for **batch updates followed by batch queries**.\n\n---\n\n##  Common Use Cases\n\n1. **Range frequency problems** (like yours)\n2. **Scheduling or booking systems** (intervals)\n3. **Event simulation** — adding effects over time intervals\n4. **Sweep line algorithms**\n5. **Histogram frequency transforms**\n\n---\n\n##  Cautions\n\n* You **must build the prefix sum** after all range updates before querying.\n* Be careful about bounds: always make sure `r+1` is in range.\n* If you're doing multiple range updates and queries *online* (interleaved), consider a segment tree or Binary Indexed Tree instead.\n\n---\n\n##  Bonus: 2D Difference Array\n\nYou can even extend this to **2D grids**:\n\nTo add `+x` to a submatrix defined by top-left $(x_1, y_1)$ and bottom-right $(x_2, y_2)$:\n\n```cpp\nD[x1][y1] += x;\nD[x2+1][y1] -= x;\nD[x1][y2+1] -= x;\nD[x2+1][y2+1] += x;\n```\n\nThen apply a 2D prefix sum over `D` to reconstruct the matrix.\n\n---\n\n## Summary\n\n| Concept              | Meaning                                       |\n| -------------------- | --------------------------------------------- |\n| What                 | Stores the *differences* between elements     |\n| Use case             | Fast range updates in constant time           |\n| Final value recovery | Prefix sum                                    |\n| Trade-off            | No direct queries until prefix sum is applied |\n| Extension            | Works in 2D too                               |\n\n",
        "category": "algo",
        "resource": {
          "in": "",
          "out": ""
      ...
      },
      {
        "id": 34,
        "title": "Prefix Arrays",
        "content": "### 1. Prefix Sum Array\nGiven an array of $n$ elements, and $q$ queries asking for the sum of elements from index $l$ to $r$, a naive brute-force approach would loop through each range, resulting in a time complexity of $O(q \\cdot (r - l))$.\n\nTo optimize this, we can preprocess a **prefix sum array** in $O(n)$ time, and then answer each query in $O(1)$ time.\n\nA prefix sum array is an array where the $i$th element represents the sum of elements from index $0$ to $i$ in the original array.\n\n#### Example\nSuppose we have the array:\n```\n1 5 3 2 5 8\n```\nAnd we are asked to find the sum from $l = 2$ to $r = 5$ (i.e., elements 5, 3, 2, 5). The manual sum is $5 + 3 + 2 + 5 = 15$.\n\nWe first build the prefix sum array:\n```\nprefixSum = [1, 6, 9, 11, 16, 24]\n```\nThis corresponds to:\n- prefixSum[0] = 1\n- prefixSum[1] = 1 + 5 = 6\n- prefixSum[2] = 1 + 5 + 3 = 9\n- ... and so on\n\nTo compute the sum from index $l$ to $r$, we use:\n$$\nsum(l, r) = prefixSum[r] - prefixSum[l - 1]\n$$\nIf $l = 0$, we simply return $prefixSum[r]$.\n\n---\n\n### 2. Prefix Multiplication Array\nGiven an array of $n$ elements, and $q$ queries asking for the **product** of elements from index $l$ to $r$, we can apply the same prefix concept.\n\nInstead of cumulative sum, we store cumulative multiplication:\n$$\nprefixProduct[i] = A[0] \\times A[1] \\times \\dots \\times A[i]\n$$\nTo get the product from $l$ to $r$:\n$$\nproduct(l, r) = \\frac{prefixProduct[r]}{prefixProduct[l - 1]}\n$$\n#### ⚠️ Important Caveats:\n- This only works if **none of the elements are zero**, because division by zero is undefined.\n- If you're working with **modulo arithmetic** (common in contests), you must use **modular inverses**:\n$$\nproduct(l, r) = prefixProduct[r] \\times inverse(prefixProduct[l - 1]) \\mod M\n$$\nwhere $M$ is a prime modulus.\n\n---\n\n### 3. Prefix XOR Array\nGiven $q$ queries asking for the XOR of elements from index $l$ to $r$, we can define a prefix XOR array such that:\n$$\nprefixXOR[i] = A[0] \\oplus A[1] \\oplus \\dots \\oplus A[i]\n$$\nTo compute the XOR from index $l$ to $r$:\n$$\nXOR(l, r) = prefixXOR[r] \\oplus prefixXOR[l - 1]\n$$\n#### Edge Case:\nIf $l = 0$, then the answer is simply:\n$$\nXOR(0, r) = prefixXOR[r]\n$$\n\nThis trick works due to the property:\n$$\na \\oplus a = 0, \\quad a \\oplus 0 = a\n$$\nSo taking XOR from $0$ to $r$, and removing XOR from $0$ to $l-1$, gives the result for $[l, r]$.\n\n---\n\n### Summary\n| Operation      | Prefix Formula                         | Query Formula                          | Notes                                      |\n|----------------|-----------------------------------------|-----------------------------------------|---------------------------------------------|\n| Sum            | $S[i] = A[0] + \\dots + A[i]$            | $S[r] - S[l - 1]$                        | Base case: if $l = 0$, use $S[r]$           |\n| Product        | $P[i] = A[0] \\times \\dots \\times A[i]$ | $P[r] / P[l - 1]$                        | Works only without zeros or with inverses   |\n| XOR            | $X[i] = A[0] \\oplus \\dots \\oplus A[i]$  | $X[r] \\oplus X[l - 1]$                   | Base case: if $l = 0$, use $X[r]$           |\n\n # 2d prefixSum\n\n### 📌 What is a 2D Prefix Sum?\n\nIn competitive programming, a **2D prefix sum** (or cumulative sum) is a powerful technique used to **quickly compute the sum of elements in a submatrix** of a 2D array.\n\n---\n\n### 🧠 Idea\n\nYou **preprocess** the 2D array in such a way that you can **answer sum queries over any subrectangle** in **constant time** $O(1)$, after $O(n \\times m)$ preprocessing time.\n\n---\n\n### 🧮 Prefix Sum Definition\n\nLet `ps[i][j]` be the sum of elements from the top-left corner `(0, 0)` to the cell `(i, j)` (inclusive).\nThe recurrence:\n\n$$\n\\text{ps}[i][j] = \\text{mat}[i][j] + \\text{ps}[i-1][j] + \\text{ps}[i][j-1] - \\text{ps}[i-1][j-1]\n$$\n\nThe subtraction avoids double-counting the overlapping part.\n\n---\n\n### ✅ Sum Query of Submatrix\n\nTo get the sum of a submatrix from `(x1, y1)` to `(x2, y2)` (inclusive):\n\n$$\n\\text{sum} = \\text{ps}[x2][y2] - \\text{ps}[x1-1][y2] - \\text{ps}[x2][y1-1] + \\text{ps}[x1-1][y1-1]\n$$\n\nHandle boundaries carefully to avoid negative indices.\n\n---\n\n### 🧪 Example Problem\n\n#### 🟦 Problem Statement:\n\nGiven a 2D grid `mat` of integers (size `n x m`) and `q` queries. Each query gives 4 integers `(x1, y1, x2, y2)` and asks for the **sum of elements inside the submatrix** from `(x1, y1)` to `(x2, y2)`.\n\n* `1 ≤ n, m ≤ 1000`\n* `1 ≤ q ≤ 10^5`\n* `-10^4 ≤ mat[i][j] ≤ 10^4`\n* `1 ≤ x1 ≤ x2 ≤ n`\n* `1 ≤ y1 ≤ y2 ≤ m`\n\n---\n\n### 🔧 Implementation (C++)\n\n```cpp\n#include <iostream>\n#include <vector>\nusing namespace std;\n\nint main() {\n    int n, m, q;\n    cin >> n >> m >> q;\n\n    vector<vector<int>> mat(n + 1, vector<int>(m + 1));\n    vector<vector<int>> ps(n + 1, vector<int>(m + 1));\n\n    // Input\n    for (int i = 1; i <= n; ++i)\n        for (int j = 1; j <= m; ++j)\n            cin >> mat[i][j];\n\n    // Precompute prefix sums\n    for (int i = 1; i <= n; ++i)\n        for (int j = 1; j <= m; ++j)\n            ps[i][j] = mat[i][j] + ps[i-1][j] + ps[i][j-1] - ps[i-1][j-1];\n\n    // Answer queries\n    while (q--) {\n        int x1, y1, x2, y2;\n        cin >> x1 >> y1 >> x2 >> y2;\n\n        int res = ps[x2][y2] - ps[x1-1][y2] - ps[x2][y1-1] + ps[x1-1][y1-1];\n        cout << res << '\\n';\n    }\n\n    return 0;\n}\n```\n\n---\n\n### 🧾 Example\n\n**Input:**\n\n```\n3 3 2\n1 2 3\n4 5 6\n7 8 9\n1 1 2 2\n2 2 3 3\n```\n\n**Explanation:**\n\n* Query 1: Sum of submatrix (1,1) to (2,2) = 1+2+4+5 = 12\n* Query 2: Sum of submatrix (2,2) to (3,3) = 5+6+8+9 = 28\n\n**Output:**\n\n```\n12\n28\n```\n\n---\n\n### 🚀 Time Complexity\n\n* Preprocessing: $O(n \\times m)$\n* Each Query: $O(1)$\n* Total for `q` queries: $O(n \\times m + q)$\n\n",
        "category": "algo",
        "resource": {
          "in": "",
          "out": ""
